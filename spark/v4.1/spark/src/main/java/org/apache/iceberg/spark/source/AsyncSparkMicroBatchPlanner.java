...
/**
 * Asynchronous implementation of {@link SparkMicroBatchPlanner} that uses background processing and
 * queuing to optimize manifest scanning.
 *
 * <p>This planner maintains a queue of pre-fetched file scan tasks and uses a background executor
 * to populate the queue asynchronously. This reduces latency during micro-batch planning by
 * overlapping manifest scanning with query execution.
 */
class AsyncSparkMicroBatchPlanner extends BaseSparkMicroBatchPlanner implements AutoCloseable {
  ...
  AsyncSparkMicroBatchPlanner(
    ...
    super(table, readConf, lastOffsetForTriggerAvailableNow);
  ...
  public synchronized void stop() {
    ...
    LOG.info(
        "AsyncSparkMicroBatchPlanner for table: {}, stopped: {}", table().name(), terminated);
  }
